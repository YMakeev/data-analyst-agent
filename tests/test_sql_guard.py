...
Дві однаково важливі половини. Перша — що перевірка мусить відбити. Друга,
про яку зазвичай забувають, — чого вона чіпати НЕ мусить.

Хибне спрацювання коштує дорожче за пропущений небезпечний запит: модель
отримує відмову на цілком нормальному питанні, не розуміє причини й починає
переписувати запит навмання. А від справді небезпечного запиту захищають
права користувача бази, а не ця перевірка.
"""
...
def test_error_message_tells_model_what_to_do() -> None:
    """Відмова пояснює, що робити натомість.

    Claude читає текст помилки й намагається виправитись сам. Сухе
    «заборонено» заганяє його в глухий кут, а «дозволені лише запити на
    читання, перепиши як SELECT» дає вихід без участі людини.
    """
...
def test_model_records_library_versions() -> None:
    """Файл моделі пам'ятає, якими бібліотеками його створили.

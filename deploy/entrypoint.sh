...
# Модель НЕ тренуємо на старті: це затримує відкриття порту, і хостинг може
# визнати деплой невдалим. Її натренує сам інструмент predict_churn при
# першому виклику (server/tools/ml.py). Розкоментуй, якщо хочеш платити цю
# затримку на старті замість першого запиту.
#         echo "==> Моделі немає, тренуюсь"
#         # Тренування читає ті самі дані звичайними SELECT, тож вистачає
#         # read-only ролі: адмінський доступ у хмарі не потрібен ніде.
#         if ! python ml/train.py; then
#             echo "!! Натренувати не вдалось. Сервер підніметься, але predict_churn"
#             echo "   повертатиме помилку, доки сервіс не перезапустять."
#         fi
# fi
